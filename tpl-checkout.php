		...
		<section class="box">
			...
			<div class="box__container">
				...
				<div class="box__selected" v-if="locationHasBoxes()">
					<h2><?php _e('Vali ladu', THEME_TEXT_DOMAIN); ?></h2>
					<p>
						<?php _e('Saadaolevaid ladusid', THEME_TEXT_DOMAIN); ?>: <strong>{{this.location.locations[this.getSelectedLocationIndex()].boxes.filter(b=>b.can_book).length}}</strong>
					</p>
					<v-select placeholder="<?php _e('Vali ladu', THEME_TEXT_DOMAIN); ?>" v-model="checkout.box" :options="this.location.locations[this.getSelectedLocationIndex()].boxes" label="name_price" :selectable="option => option.can_book">
				...
				<div class="extras">
					...
					<div class="extras__field" v-if="checkout.box && checkout.location && checkout.location.extras.length <= 0">
						<p><?php _e('Antud laol puudvad lisateenused', THEME_TEXT_DOMAIN); ?></p>
					</div>
				</div>
			</div>
		</section>
