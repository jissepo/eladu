<section id="vue--checkout">
		...
		<form action="<?php echo admin_url('admin-post.php') ?>" method="post" v-on:submit.prevent="submitPurchase">
			...
			<section class="extras" v-if="">
				...
				<div class="extras__container">
					...
					<div class="extras__field" v-if="boxExtras.length <= 0">
						<p><?php _e('Esmalt vali ladu', THEME_TEXT_DOMAIN); ?></p>
					</div>
				</div>
			</section>
			<section class="checkout" v-if="checkout.box || canShowContact">
	...
	<div class="container__right" sticky-container>
		<div class="confirmation__row" v-sticky sticky-offset="{top: 100, bottom: 30}" sticky-side="both" >
			<p><strong><?= __('Summa', THEME_TEXT_DOMAIN); ?>:</strong> <span :inner-html.prop="totalSum | formatSum"></span> <span v-if="totalSum !== 0">(<?php _e('sisaldab 20% käibemaksu', THEME_TEXT_DOMAIN); ?>)</span></p>
		</div>
	</div>
</section>