...
export default {
  ...
  computed: {
    ...
    monthsCount() {
      const months = this.getMonthDiff(this.checkIn, this.hoveringDate);
      const days = this.countDaysA(this.getSameDayOfMonth(this.hoveringDate, this.checkIn), this.hoveringDate)
      if (months === 0 || (months === 1 && days >= 0)) {
        const firstMonthDays = this.countDays(this.checkIn, this.hoveringDate)
        return `${firstMonthDays}&nbsp;${firstMonthDays !== 1 ? this.options.i18n['nights'] : this.options.i18n['night']}`;
      }
      const firstMonthDays = this.countDaysA(this.getSameDayOfMonth(this.hoveringDate, this.checkIn), this.hoveringDate)

      if (firstMonthDays >= 0) {
        const lastMonthDays = this.countDays(this.getSameDayOfMonth(this.getPreviousMonth(this.hoveringDate), this.checkIn), this.hoveringDate)
        return `${months-1}&nbsp;${months-1 !== 1 ? this.options.i18n['months'] : this.options.i18n['month']} ${Math.abs(lastMonthDays)}&nbsp;${lastMonthDays !== 1 ? this.options.i18n['nights'] : this.options.i18n['night']}`;

      }
      return `${months}&nbsp;${months !== 1 ? this.options.i18n['months'] : this.options.i18n['month']} ${Math.abs(firstMonthDays)}&nbsp;${Math.abs(firstMonthDays) !== 1 ? this.options.i18n['nights'] : this.options.i18n['night']}`;
    },
  ...
  methods: {
    ...
    dayClicked(date) {
      if (this.isDisabled || !this.isClickable()) {
        this.$emit('disabled-day-clicked');
        return
      }
      else {
        if (this.options.allowedRanges.length !== 0) {
