...
class ThemeSetup
{
	...
	public function __construct()
	{
		...
		add_shortcode('jj_checkout', array($this, 'shortcode_html'));
		add_shortcode('jj-get-free-boxes-count', array($this, 'get_free_boxes_count'));
	}
	...
	{
		add_action('after_setup_theme', array($this, 'jj_theme_setup'));
		add_action('wp_enqueue_scripts', array($this, 'jj_theme_enqueue_scripts_and_styles'));

		add_action('wp_ajax_get_makecommerce_data', array($this, 'get_makecommerce_data'));
		add_action('wp_ajax_nopriv_get_makecommerce_data', array($this, 'get_makecommerce_data'));

		add_action('wp_ajax_get_storage_locations', array($this, 'get_storage_locations'));
		add_action('wp_ajax_nopriv_get_storage_locations', array($this, 'get_storage_locations'));
		...
		add_action('wp_ajax_get_available_countries', array($this, 'get_available_countries'));
		add_action('wp_ajax_nopriv_get_available_countries', array($this, 'get_available_countries'));

		add_action('wp_ajax_get_available_boxes', array($this, 'get_available_boxes'));
		add_action('wp_ajax_nopriv_get_available_boxes', array($this, 'get_available_boxes'));

		add_action('wp_ajax_send_verification_code', array($this, 'send_verification_code'));
		add_action('wp_ajax_nopriv_send_verification_code', array($this, 'send_verification_code'));

		add_action('wp_ajax_validate_phone', array($this, 'validate_phone'));
		add_action('wp_ajax_nopriv_validate_phone', array($this, 'validate_phone'));

		add_action('admin_post_send_cron_keys', array($this, 'send_cron_keys'));
		add_action('admin_post_nopriv_send_cron_keys', array($this, 'send_cron_keys'));
		...
		wp_localize_script(
			...
			array(
				...
				'translations'  => array(
					...
					'delayed'    => \get_field('delay_string'),
				),
			)
		);
	}
...
<?php
		...
		if (count($product_ids)) {
			...
			foreach ($product_ids as $key => $post_id) {
				...
				$returnValue[] = array(
					...
					'image'              => wp_get_attachment_image_src(get_post_thumbnail_id($product->get_id()), 'large')[0],
	...
	public function send_verification_code()
	{
		...
		if ($mobile && \strpos($mobile, '+372') !== false && \strlen(\trim($mobile)) > strlen('+372')) {
			...
			$verfication_code = \rand(1000, 9999);
			...
			foreach ($recipients as $msisdn) {
				$json['recipients'][] = array('msisdn' => $msisdn);
			}
			...
			// $result = curl_exec($ch);
			curl_close($ch);
			// wp_send_json_success(array('message' => __('Teie telefoni on saadetud sõnum valideerimis koodiga.', THEME_TEXT_DOMAIN)));
			wp_send_json_success(
				array(
					'message' => __('Teie telefoni on saadetud sõnum valideerimis koodiga.', THEME_TEXT_DOMAIN),
					'verify'  => $verfication_code,
				)
			);
	...
	{
		$keys_cron = get_option('jj_keys_cron', array());
		foreach ($keys_cron as $timestamp => $boxes) {
			...
			foreach ($boxes as $key => $value) {
				...
				$recipients = array(\get_user_meta($user_id, 'billing_phone', true));
				$url        = 'https://gatewayapi.com/rest/mtsms';
				$api_token  = $this->api_key;
				$json       = array(
					'sender'     => 'Eladu OÜ',
